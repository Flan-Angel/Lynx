# Lynx
Just a lot of links i wanna keep in one place

GITHUB REPOS

-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
-[Awesome](https://github.com/sindresorhus/awesome) - List of all "Awesome" github reops

-[Awesome-OSINT](https://github.com/jivoi/awesome-osint)

-[Awesome Falsehoods](https://github.com/kdeldycke/awesome-falsehood) - 
-[]() -
-[]() -
-[]() -
-[]() -





-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
-[Build Your Own _](https://github.com/codecrafters-io/build-your-own-x) - Build your own

-[Algorithms](https://github.com/thealgorithms) - Open Source resource for learning Data Structures & Algorithms and their implementation in any Programming Language

-[Free-dev](https://github.com/ripienaar/free-for-dev) -  Free stuff for devs

-[Free programming Books](https://github.com/EbookFoundation/free-programming-books) -

-[Programming playgrounds](https://github.com/EbookFoundation/free-programming-books/blob/main/more/free-programming-playgrounds.md) - Run code online or something

-[Roadmaps](https://github.com/kamranahmedse/developer-roadmap) - Roadmaps for stuff 

-[CompSci](https://github.com/ossu/computer-science) - Teach yourself Computer science

-[Engineering Blogs](https://github.com/kilimchoi/engineering-blogs) - 

-[Design Large Scale Systems](https://github.com/donnemartin/system-design-primer) - 

-[Public APIs](https://github.com/public-apis/public-apis) - 

-[Open Source Alternatives](https://github.com/btw-so/open-source-alternatives) - 

-[Computer Science Papers](https://github.com/papers-we-love/papers-we-love) - 

-[Awesome Websites](https://github.com/sdmg15/Best-websites-a-programmer-should-visit) - 

-[LLMs From 0](https://github.com/rasbt/LLMs-from-scratch) - 

-[Machine Learning](https://github.com/eriklindernoren/ML-From-Scratch) - From Scratch 

-[More Machine Learning](https://github.com/GokuMohandas/Made-With-ML) - 

-[DevOps Interview Questions](https://github.com/NotHarshhaa/DevOps-Interview-Questions) -

-[GitHub Profile Achievements](https://github.com/Schweinepriester/github-profile-achievements) - A collection listing all Achievements available on the GitHub profile.

-[Linux PDF Thing](https://github.com/ading2210/linuxpdf) - Linux running inside a PDF file via a RISC-V emulator 

-[GitHub Résumé](https://github.com/resume/resume.github.com) - A service that creates a résumé based on your GitHub repos/activity.

-[Markdown To PDF converter](https://github.com/realdennis/md2pdf) -

-[Developer Chestsheets](https://github.com/Fechin/reference) -

-[More Developer Cheatsheets](https://github.com/ivansaul/cheat-sheets) -

-[CSS instead of Javascript](https://github.com/you-dont-need/You-Dont-Need-JavaScript) -

-[Infinite loop in all Languages](https://github.com/In-All-Programming-Languages/infinite-loop-in-all-programming-languages) -

-[You Don't Need](https://github.com/you-dont-need/You-Dont-Need) - Things you dont need

-[Programming Books For Free](https://github.com/EbookFoundation/free-programming-books) -

-[Tauri](https://github.com/tauri-apps) - Build smaller, faster, and more secure desktop applications with a web frontend

-[Secret Knowledge Book](https://github.com/trimstray/the-book-of-secret-knowledge) - A collection of inspiring lists, manuals, cheatsheets, blogs, hacks, one-liners, cli/web 
tools and more. 

-[Papers we Love](https://github.com/papers-we-love/papers-we-love) - Papers from the computer science community to read and discuss. 

-[]() -

-[]() -

-[]() -

-[]() -

-[]() -

-[]() -

-[]() -





-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
LINUX RESOURCES from that Bread on Penguins vid


-[Linux Filesystem DeepDive](https://refspecs.linuxfoundation.org/FHS_3.0/fhs/index.html)

-[SuperGrub2Disk](https://www.supergrubdisk.org/super-grub2-disk/) - For when your linux bootloader is broken and you need to boot your system

-[RescaTux](https://www.supergrubdisk.org/category/download/rescatuxdownloads/rescatux-stable/) - For when your Linux/Windows bootloader is broken and you need to boot your system

-[CommandLineFu](https://www.commandlinefu.com/commands/browse) - Community that shares cool commandline stuff they found

-[Linux Documentation](https://www.commandlinefu.com/commands/browse) - Better Linux Docs 

-[PureShBible](https://github.com/dylanaraps/pure-sh-bible) - Resource for POSIX sh alternatives

-[PureBashBible](https://github.com/dylanaraps/pure-bash-bible) - Resource for POSIX Bash alternatives

-[ShellCheck](https://www.shellcheck.net/) - Checks your Shell_Scripts for bugs

-[LinuxPerformance](https://www.brendangregg.com/linuxperf.html) 

-[TealDeer](https://github.com/tealdeer-rs/tealdeer) - Examples for ManPages. TLDR but written in rust cause uhhh SPEED

-[WarGames](https://overthewire.org/wargames/) Practice System_Security concepts using games

-[]()




-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
WIKIS!!!



-[Gentoo](https://wiki.gentoo.org/wiki/Main_Page)

-[ARCH!!!!](https://wiki.archlinux.org/title/Main_page) - The DuckDuckGo Bang for the ArchWiki is !aw and !alp for ArchLinuxPackages

-[Debian](https://wiki.debian.org/)



-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
COOL GAME RELATED STUFF

-[Messanger](https://messenger.abeto.co/) - Cool game that runs in your browser

-[Open Source Games](https://github.com/bobeff/open-source-games) - 

-[Awesome Game Remakes](https://github.com/radek-sprta/awesome-game-remakes) - 

-[Awesome Open-Source Games](https://github.com/michelpereira/awesome-open-source-games) - 

-[Open-Source Games On Github](https://github.com/leereilly/games) - 

-[Libre Games Wiki](https://libregamewiki.org/Main_Page) - 

-[FOSS Game engine replacements](https://forum.openmw.org/viewtopic.php?p=57882) - 

-[More Open Source Games](https://trilarion.github.io/opensourcegames/) -

-[Mac Source Ports](https://www.macsourceports.com/) - Run old games on new Macs

-[Open-Source Game clones](https://osgameclones.com/) -

-[]() -

-[]() -

-[]() -

-[]() -

-[]() -

-[]() -

-[]() -

-[]() -

-[]() -

-[]() -

-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
PRIVACY
I don't have anything to hide, but I don't have anything to show you either.

-[Degoogle](https://git.tycrek.com/archive/degoogle) - ALternatives for Google

-[Privacy Guides](https://discuss.privacyguides.net/c/community-wiki/9411/none) -

-[Tuta Mail](https://mail.tutanota.com) - Private E-Mail service 

-[TailsOS](https://tails.net/) - is a portable operating system
that protects against
surveillance and censorship. 

-[Tor](https://www.torproject.org/download/) - Browser to protect yourself against tracking, surveillance, and censorship.

-[]() -

-[]() -

-[]() -

-[]() 

-[]() -

-[]() -

-[]() -

-[]() -










